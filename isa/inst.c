...
#define Rs()   do { rs = (Mr(MEM_SFR_PSW, MEM_TYPE_IRAM) >> 3) & 0x03 ; } while(0)
...
#define SP_INC()  Mw(MEM_SFR_SP, Ir(MEM_SFR_SP) + 1, MEM_TYPE_IRAM)
#define SP_SUB()  Mw(MEM_SFR_SP, Ir(MEM_SFR_SP) - 1, MEM_TYPE_IRAM)

#define Or0()   do { or0 = Ir(op0); } while(0)
#define Or1()   do { or1 = Ir(op1); } while(0)

#define ADD_PSW_CY() do { Mw(MEM_SFR_PSW_CY, (add & 0x100 ? 1 : 0), MEM_TYPE_BIT); } while(0)
#define ADD_PSW_AC() do { Mw(MEM_SFR_PSW_AC, ((((or0 & 0xf) + (or1 & 0xf)) & 0x10) ? 1 : 0), MEM_TYPE_BIT); } while(0)
#define ADD_PSW_OV() do { word_t bit7_c = add >= 0x100 ? 1 : 0; word_t bit6_c = (((or0 & 0x7F) + (or1 & 0x7F)) >= 0x80) ? 1 : 0; \
    Mw(MEM_SFR_PSW_OV, bit7_c ^ bit6_c, MEM_TYPE_BIT); } while(0)

#define SUB_PSW_CY() do { Mw(MEM_SFR_PSW_CY, (or0 < or1 ? 1 : 0), MEM_TYPE_BIT); } while(0)
#define SUB_PSW_AC() do { Mw(MEM_SFR_PSW_AC, ((or0 & 0xf) < (or1 & 0xf) ? 1 : 0), MEM_TYPE_BIT); } while(0)
#define SUB_PSW_OV() do { word_t bit7_c = (or0 < or1) ? 1 : 0; word_t bit6_c = ((or0 & 0x7f) < (or1 & 0x7f)) ? 1 : 0; \
    Mw(MEM_SFR_PSW_OV, bit7_c ^ bit6_c, MEM_TYPE_BIT); } while(0)

#define Xchd()  do { lor0 = or0 & 0x0F; lor1 = or1 & 0x0F; \
    or0 = (or0 & 0xF0) | lor1; or1 = (or1 & 0xF0) | lor0;} while(0)
#define Swap(i) do { i = ((i & 0xf0) >> 4) | ((i & 0x0f) << 4); } while(0)
#define RL()    do { or0 = ((or0 & 0x80) >> 7) | (or0 << 1); } while(0)
#define RR()    do { or0 = ((or0 & 0x01) << 7) | (or0 >> 1); } while(0)
#define RLC()   do { or1 = (or0 & 0x80) >> 7; or0 = Br(MEM_SFR_PSW_CY) | (or0 << 1); } while(0)
#define RRC()   do { or1 = or0 & 0x01; or0 = (Br(MEM_SFR_PSW_CY) << 7) | (or0 >> 1); } while(0)
#define Add()   do { dword_t add = or0 + or1; if (inst_encode->inst_byte0 & 0x10) add += Br(MEM_SFR_PSW_CY); \
    ADD_PSW_AC(); ADD_PSW_CY(); ADD_PSW_OV(); or0 = (word_t) add; } while(0)
#define Sub()   do { or1 += Br(MEM_SFR_PSW_CY); SUB_PSW_AC(); SUB_PSW_CY(); SUB_PSW_OV(); or0 -= or1; } while(0)
#define Mul()   do { dword_t mul = or0 * or1; Mw(MEM_SFR_PSW_OV, (mul > 255 ? 1 : 0), MEM_TYPE_BIT); \
    Mw(MEM_SFR_PSW_CY, 0, MEM_TYPE_BIT); or0 = mul & 0xff; or1 = mul >> 8; } while(0)
#define Div()   do { if (or1 == 0) Mw(MEM_SFR_PSW_OV, 1, MEM_TYPE_BIT);  else { word_t div = or0 / or1; \
    or1 = or0 % or1; or0 = div; Mw(MEM_SFR_PSW_OV, 0 , MEM_TYPE_BIT); } Mw(MEM_SFR_PSW_CY, 0 , MEM_TYPE_BIT); } while(0)
#define Da()    do { word_t ac = Br(MEM_SFR_PSW_AC); word_t cy = Br(MEM_SFR_PSW_CY); dword_t acc = or0; \
    if ((acc & 0x0f) > 9 || ac) acc  += 0x6; if (acc >= 0x100) cy = 1; \
    if ((acc & 0xf0) > 0x90 || cy) acc  += 0x60; if (acc >= 0x100) cy = 1; \
    or0 = (word_t)acc; Mw(MEM_SFR_PSW_CY, cy, MEM_TYPE_BIT); } while(0)

static void Call(inst_encode_t *inst_encode, char Type) {
  word_t sp = Sr(MEM_SFR_SP) + 1;
  Mw(sp++, (word_t)(mcu.pc & 0xff), MEM_TYPE_IRAM);
  Mw(sp  , (word_t)(mcu.pc >>   8), MEM_TYPE_IRAM);
...
void decode_operand(inst_encode_t *inst_encode, dword_t *op0, dword_t *op1, 
  ...
  switch (op0_mode) {
    ...
    case OP_ADDR_MODE_REG_ACC:              *op0 = MEM_SFR_ACC;                   break;
    case OP_ADDR_MODE_REG_C :               *op0 = MEM_SFR_PSW_CY;                break;
    case OP_ADDR_MODE_REG_DP:               *op0 = MEM_SFR_DPH;                   break;
    case OP_ADDR_MODE_IMM_8 :               *op0 = inst_encode->inst_byte1;       break;
    case OP_ADDR_MODE_IMM_H :               *op0 = inst_encode->inst_byte1;       break;
    case OP_ADDR_MODE_DIRECT:               *op0 = inst_encode->inst_byte1;       break;
    case OP_ADDR_MODE_IND_RI:   Rs(); Ri(); *op0 = Mr(rs + ri, MEM_TYPE_IRAM);    break;
    case OP_ADDR_MODE_IND_A_DP: DPTR();     *op0 = Ir(MEM_SFR_ACC) + dptr ;       break;
    case OP_ADDR_MODE_IND_A_PC:             *op0 = Ir(MEM_SFR_ACC) + mcu.pc + 1;  break;
    case OP_ADDR_MODE_IND_DPTR: DPTR();     *op0 = dptr;                          break;
    case OP_ADDR_MODE_BIT :                 *op0 = inst_encode->inst_byte1;       break;
    case OP_ADDR_MODE_NONE: break;
  }

  switch (op1_mode) {
    case OP_ADDR_MODE_REG_RN:   Rs(); Rn(); *op1 = rs + rn;                       break;  
    case OP_ADDR_MODE_REG_ACC:              *op1 = Ir(MEM_SFR_ACC);               break;
    case OP_ADDR_MODE_REG_C :               *op1 = MEM_SFR_PSW_CY;                break;
    case OP_ADDR_MODE_REG_BR:               *op1 = MEM_SFR_B;                     break;
    case OP_ADDR_MODE_REG_DP:               *op1 = MEM_SFR_DPL;                   break;
    case OP_ADDR_MODE_IMM_8:                *op1 = bytes == 3 ? ibyte2 : ibyte1;  break;
    case OP_ADDR_MODE_IMM_H :               *op1 = inst_encode->inst_byte2;       break;
    case OP_ADDR_MODE_DIRECT:               *op1 = bytes == 3 ? ibyte2 : ibyte1;  break;
    case OP_ADDR_MODE_IND_RI:   Rs(); Ri(); *op1 = Mr(rs + ri, MEM_TYPE_IRAM);    break; 
    case OP_ADDR_MODE_IND_A_DP: DPTR();     *op1 = Ir(MEM_SFR_ACC) + dptr ;       break;
    case OP_ADDR_MODE_IND_A_PC:             *op1 = Ir(MEM_SFR_ACC) + mcu.pc + 1;  break;
    case OP_ADDR_MODE_IND_DPTR: DPTR();     *op1 = dptr;                          break;   
    case OP_ADDR_MODE_BIT :                 *op1 = bytes == 3 ? ibyte2 : ibyte1;  break;
    case OP_ADDR_MODE_NONE: break;
  }
  // printf("op0 = %x, op1 = %x\n", *op0, *op1);
...
void decode_exec(inst_encode_t *inst_encode) {
  dword_t op0, op1; dword_t bytes, cycles; word_t or0, or1, lor0, lor1;
  word_t opcode = inst_encode->inst_byte0;
  word_t ibyte1 = inst_encode->inst_byte1;
  word_t ibyte2 = inst_encode->inst_byte2;
...
#define INSTPAT_MATCH(inst_encode, bytes, cycles, name, op0_mode, op1_mode,  ... ) {  \
  ...
  INSTPAT("1100 0100", 1, 1, SWAP A                 , REG_ACC , NONE    , Or0(); Swap(or0); Mw(op0, or0, MEM_TYPE_IRAM));
  ...
  INSTPAT("0010 1???", 1, 1, ADD  A       Rn        , REG_ACC , REG_RN  , Or0(); Or1(); Add(); Mw(op0, or0, MEM_TYPE_IRAM));
  INSTPAT("0010 0101", 2, 1, ADD  A       direct    , REG_ACC , DIRECT  , Or0(); Or1(); Add(); Mw(op0, or0, MEM_TYPE_IRAM));
  INSTPAT("0010 011?", 1, 1, ADD  A       @Ri       , REG_ACC , IND_RI  , Or0(); Or1(); Add(); Mw(op0, or0, MEM_TYPE_IRAM));
  INSTPAT("0010 0100", 2, 1, ADD  A       #data     , REG_ACC , IMM_8   , Or0(); or1 = op1; Add(); Mw(op0, or0, MEM_TYPE_IRAM));

  INSTPAT("0011 1???", 1, 1, ADDC A       Rn        , REG_ACC , REG_RN  , Or0(); Or1(); Add(); Mw(op0, or0, MEM_TYPE_IRAM));
  INSTPAT("0011 0101", 2, 1, ADDC A       direct    , REG_ACC , DIRECT  , Or0(); Or1(); Add(); Mw(op0, or0, MEM_TYPE_IRAM));
  INSTPAT("0011 011?", 1, 1, ADDC A       @Ri       , REG_ACC , IND_RI  , Or0(); Or1(); Add(); Mw(op0, or0, MEM_TYPE_IRAM));
  INSTPAT("0011 0100", 2, 1, ADDC A       #data     , REG_ACC , IMM_8   , Or0(); or1 = op1; Add(); Mw(op0, or0, MEM_TYPE_IRAM));

  INSTPAT("0000 0100", 1, 1, INC  A                 , REG_ACC , NONE    , Or0(); or1 = 0x1; Add(); Mw(op0, or0, MEM_TYPE_IRAM));
  ...
  INSTPAT("1001 1???", 1, 1, SUBB A       Rn        , REG_ACC , REG_RN  , Or0(); Or1(); Sub(); Mw(op0, or0, MEM_TYPE_IRAM));
  INSTPAT("1001 0101", 2, 1, SUBB A       direct    , REG_ACC , DIRECT  , Or0(); Or1(); Sub(); Mw(op0, or0, MEM_TYPE_IRAM));
  INSTPAT("1001 011?", 1, 1, SUBB A       @Ri       , REG_ACC , IND_RI  , Or0(); Or1(); Sub(); Mw(op0, or0, MEM_TYPE_IRAM));
  INSTPAT("1001 0100", 2, 1, SUBB A       #data     , REG_ACC , IMM_8   , Or0(); or1 = op1; Sub(); Mw(op0, or0, MEM_TYPE_IRAM));

  INSTPAT("1010 0100", 1, 4, MUL  AB                , REG_ACC , REG_BR  , Or0(); Or1(); Mul(); Mw(op0, or0, MEM_TYPE_IRAM); Mw(op1, or1, MEM_TYPE_IRAM));
  INSTPAT("1000 0100", 1, 4, DIV  AB                , REG_ACC , REG_BR  , Or0(); Or1(); Div(); Mw(op0, or0, MEM_TYPE_IRAM); Mw(op1, or1, MEM_TYPE_IRAM));
  INSTPAT("1101 0100", 1, 1, DA   A                 , REG_ACC , NONE    , Or0();        Da();  Mw(op0, or0, MEM_TYPE_IRAM));
  ...
  INSTPAT("1101 1???", 2, 2, DJNZ Rn      rel       , REG_RN  , NONE    , Or0(); Mw(op0, or0 - 1, MEM_TYPE_IRAM); mcu.pc += or0 != 1 ? (int8_t)ibyte1 : 0);
  INSTPAT("1101 0101", 3, 2, DJNZ direct  rel       , DIRECT  , NONE    , Or0(); Mw(op0, or0 - 1, MEM_TYPE_IRAM); mcu.pc += or0 != 1 ? (int8_t)ibyte2 : 0);
